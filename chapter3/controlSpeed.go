...
func NewController(ratePerSecond int) *Controller {
	...
	//log.Println(ratePerSecond, capacity, ratePerSecond*int(Window), time.Second)
	capacity := ratePerSecond / 20
	self := &Controller{
		threshold: capacity,
		capacity:  capacity,
...
func main() {
	...
	dst, _ = os.OpenFile("defer4", os.O_WRONLY|os.O_CREATE, 0644)

	wc := NewRateWriter(dst, speed)
	defer wc.Close()
